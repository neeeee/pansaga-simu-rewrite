function Calc(a, b) {
  ...
  if (!b) {
    switch (a) {
      ...
      case "SPR": // [  4] 霊感
        tmp[0] = 4;
        tmp[1] +=
          Status["Job"][2] >= 15 && Status["Job"][2] <= 17
            ? 1 +
              Math.floor((Status["Skill"][12][0] + Status["Skill"][12][1]) / 10)
            : 0;
        break;
      case "INT": // [  5] 知性
        tmp[0] = 5;
    ...
    switch (a) {
      case "LP": // [  6] LP
        tmp[0] = 6;
        tmp[1] =
          Status["Mod"][Status["Job"][2]][0] +
          Math.ceil(
            Status["Lev"][0] * (100 / Status["Mod"][Status["Job"][2]][2]),
          ) +
          Math.ceil(
            (Status["STA"][0] + Status["STA"][1] + Status["STA"][2]) *
              Math.ceil(100 / Status["Mod"][Status["Job"][2]][4]),
          );
        tmp[2] += Flag["14_1"] ? Math.ceil(250 + tmp[1] * 0.15) : 0; // ディバインエイド
        if ($("SelBuffClan_1").selectedIndex == 1)
          tmp[2] += Math.ceil(tmp[1] * 0.02); // フィジカル
        else if ($("SelBuffClan_1").selectedIndex == 2)
          tmp[2] += Math.ceil(tmp[1] * 0.04);
        else if ($("SelBuffClan_1").selectedIndex == 3)
          tmp[2] += Math.ceil(tmp[1] * 0.06);
        else if ($("SelBuffClan_1").selectedIndex == 4)
          tmp[2] += Math.ceil(tmp[1] * 0.09);
        else if ($("SelBuffClan_1").selectedIndex == 5)
          tmp[2] += Math.ceil(tmp[1] * 0.12);
        if (EquipOpt[tmp[0]]) {
          for (i = 0; i < EquipOpt[tmp[0]].length; i++) {
            if (EquipOpt[tmp[0]][i].search(/%/i) != -1)
              tmp[2] += Math.ceil(
                (tmp[1] * Number(EquipOpt[tmp[0]][i].slice(0, -1))) / 100,
              );
            else tmp[2] += Number(EquipOpt[tmp[0]][i]);
          }
        }
        tmp[1] += tmp[2];
        Status["LP"] = tmp[1];
      ...
      case "MP": // [  7] MP
        tmp[0] = 7;
        tmp[1] =
          Status["Mod"][Status["Job"][2]][1] +
          Math.ceil(
            Status["Lev"][0] * (100 / Status["Mod"][Status["Job"][2]][3]),
          ) +
          Math.ceil(
            (Status["SPR"][0] + Status["SPR"][1] + Status["SPR"][2]) *
              Math.ceil(100 / Status["Mod"][Status["Job"][2]][5]),
          );
        if ($("SelBuffClan_2").selectedIndex == 1)
          tmp[2] += Math.ceil(tmp[1] * 0.02); // スピリット
        else if ($("SelBuffClan_2").selectedIndex == 2)
          tmp[2] += Math.ceil(tmp[1] * 0.04);
        else if ($("SelBuffClan_2").selectedIndex == 3)
          tmp[2] += Math.ceil(tmp[1] * 0.06);
        else if ($("SelBuffClan_2").selectedIndex == 4)
          tmp[2] += Math.ceil(tmp[1] * 0.09);
        else if ($("SelBuffClan_2").selectedIndex == 5)
          tmp[2] += Math.ceil(tmp[1] * 0.12);
        if (EquipOpt[tmp[0]]) {
          for (i = 0; i < EquipOpt[tmp[0]].length; i++) {
            if (EquipOpt[tmp[0]][i].search(/%/i) != -1)
      ...
      case "POT": // [  8] ポーション回復量
        ...
        tmp[1] +=
          ...
          Status["Job"][2] >= 0 &&
          Status["Job"][2] <= 6
            ? Math.floor((Status["Skill"][0][0] + Status["Skill"][0][1]) / 10) *
              10
            : 0; // 回復量増加
      ...
      case "ATK": // [ 18] 攻/最大
        tmp[0] = 18;
        tmp[1] = Math.floor(
          (Status["STR"][0] + Status["STR"][1] + Status["STR"][2]) / 2,
        );
        tmp[2] = new Array(
          Math.floor(Status["Equip"][0][0] / 10000),
          Status["Equip"][0][0] % 10000,
        );
        ...
        // 筋力ボーナス
        if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 130)
          tmp[3] = 0.9;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 120)
          tmp[3] = 0.75;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 110)
          tmp[3] = 0.6;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 100)
          tmp[3] = 0.5;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 90)
          tmp[3] = 0.4;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 80)
          tmp[3] = 0.32;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 70)
          tmp[3] = 0.24;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 60)
          tmp[3] = 0.18;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 50)
          tmp[3] = 0.12;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 40)
          tmp[3] = 0.08;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 30)
          tmp[3] = 0.04;
        else if (Status["STR"][0] + Status["STR"][1] + Status["STR"][2] >= 20)
          tmp[3] = 0.02;
        else tmp[3] = 0;
        ...
        tmp[4] = // 闘争心
          ...
          tmp[6] == 12
            ...
            : 0;
        var ATK =
          Status["Equip"][0][0] % 10000 != 0
            ? tmp[1] + xx + tmp[2] + (xx + tmp[2]) * tmp[3] + tmp[4]
            : 1 + tmp[1];
        tmp[7] = Flag["Honor"] == 7 ? 10 : 0;
        if (Flag[7]) {
          if (Status["Job"][2] == 3 || Status["Job"][2] == 6)
            tmp[7] +=
              50 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 50 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 10)
            tmp[7] +=
              30 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 30 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 27)
            tmp[7] +=
              40 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 40 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else
            tmp[7] +=
              10 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 10 + tmp[7] + Status["Skill"][24][0] + Status["Skill"][24][1];
        } else tmp[7] = 100;
        ...
        tmp[5] += Flag["0_11"]
          ? Math.ceil(
              (ATK *
                (55 +
                  (Status["Skill"][0][0] +
                    Status["Skill"][0][1] / Math.ceil(100 / 33)))) /
                100,
            )
          : 0; // デスペラード
        if ($("SelBuffClan_10").selectedIndex == 1)
          tmp[5] += Math.ceil(ATK * 0.02); // アラウズ
        else if ($("SelBuffClan_10").selectedIndex == 2)
          tmp[5] += Math.ceil(ATK * 0.04);
        else if ($("SelBuffClan_10").selectedIndex == 3)
          tmp[5] += Math.ceil(ATK * 0.06);
        tmp[5] += Flag["Honor"] == 3 ? Math.floor(ATK * 0.02) : 0; // 脈動する猛火への道標
        if (EquipOpt[tmp[0]]) {
          for (i = 0; i < EquipOpt[tmp[0]].length; i++) {
            if (EquipOpt[tmp[0]][i].search(/%/i) != -1)
              tmp[5] += Math.ceil(
                ATK * Number(EquipOpt[tmp[0]][i].slice(0, -1) / 100),
              );
            else
              tmp[5] +=
                EquipOpt[tmp[0]][i].search(/W/i) == -1
                  ? Number(EquipOpt[tmp[0]][i])
                  : 0;
          }
        }
      ...
      case "MATK": // [ 42] 魔法攻撃力
        ...
        tmp[1] = 100;
        // 知性による魔攻ボーナス
        if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 115)
          tmp[1] += 50.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 110)
          tmp[1] += 49.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 105)
          tmp[1] += 41.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 100)
          tmp[1] += 40.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 95)
          tmp[1] += 35.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 90)
          tmp[1] += 34.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 85)
          tmp[1] += 29.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 80)
          tmp[1] += 28.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 75)
          tmp[1] += 24.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 70)
          tmp[1] += 23.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 65)
          tmp[1] += 20.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 60)
          tmp[1] += 19.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 55)
          tmp[1] += 16.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 50)
          tmp[1] += 15.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 45)
          tmp[1] += 12.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 40)
          tmp[1] += 11.25;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 35)
          tmp[1] += 8.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 30)
          tmp[1] += 7.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 25)
          tmp[1] += 5.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 20)
          tmp[1] += 4.75;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 15)
          tmp[1] += 3.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 10)
          tmp[1] += 2.0;
        else if (Status["INT"][0] + Status["INT"][1] + Status["INT"][2] >= 5)
          tmp[1] += 1.0;
        // 魔力上昇
        tmp[1] +=
          Status["Job"][2] >= 22 && Status["Job"][2] <= 24
            ? 10 +
              Math.floor((Status["Skill"][17][0] + Status["Skill"][17][1]) / 10)
            : 0;
        tmp[2] = new Array(
          Math.floor(Status["Equip"][0][0] / 10000),
          Status["Equip"][0][0] % 10000,
        ...
        // 武器のボーナス 両手杖
        if (EquipData[0][tmp[2][0]][tmp[2][1]][0] == "カースドロッド")
          tmp[1] += 20;
        else if (EquipData[0][tmp[2][0]][tmp[2][1]][0] == "アイアンスタッフ")
          tmp[1] += 0;
        else if (EquipData[0][tmp[2][0]][tmp[2][1]][0] == "マイトスタッフ")
          tmp[1] += 0;
        else tmp[1] += tmp[2][0] == 13 ? 15 : 0;
        if ($("SelBuffClan_10").selectedIndex == 1)
          tmp[1] += 2; // アラウズ
        ...
        //        tmp[1] += tmp[2];
        $("Status_" + tmp[0]).innerHTML =
          tmp[1] >= 100 ? Math.floor(tmp[1] * 10) / 10 : tmp[1];
        break;
      case "DEF": // [ 49] 防御力
        tmp[0] = 49;
        if (EquipOpt[tmp[0]])
        ...
        tmp[2] += Flag["14_4"]
          ? 5 +
            Math.floor(Number($("InBuff_0").value) / (100 / 3)) +
            Math.floor(Number($("InBuff_1").value) / (100 / 4))
          : 0; // ラピスメディオ
        tmp[1] += tmp[2];
      ...
      case "RESPhyDAM": // [ 52] 被物理ダメージ
        ...
        tmp[3] = 0;
        tmp[3] += Flag["14_6"]
          ? (10 + Math.floor(Number($("InBuff_1").value) / 5)) * -1
          : 0; // ウィンクルム
      ...
      case "ACC": // [ 62] 命中                           [ 63] 正面から攻撃した時の命中 [ 64] 背後から攻撃した時の命中
        ...
        tmp[4] = 0;
        tmp[2] +=
          Status["Job"][2] >= 8 && Status["Job"][2] <= 10
            ? 10 +
              Math.floor(Status["Skill"][6][0] + Status["Skill"][6][1] / 10)
            : 0; // 鷹の目
        tmp[2] += Flag["1_4"] ? 100 : 0; // コンセントレーション
        tmp[2] += Flag["6_3"] ? Math.floor(20 + tmp[1] * 0.15) : 0; // マインドサイト
        tmp[2] += Flag["16_0"]
          ? 5 +
            Math.floor(Number($("InBuff_0").value) / Math.ceil(100 / 13)) +
            Math.floor(Number($("InBuff_2").value) / Math.ceil(100 / 13))
          : 0; // 詩人の歌
          ...
          if (EquipOpt[tmp[0][i]]) {
            for (j = 0; j < EquipOpt[tmp[0][i]].length; j++) {
              if (EquipOpt[tmp[0][i]][j].search(/%/i) != -1)
                tmp[i + 2] += Math.ceil(
                  (tmp[1] * Number(EquipOpt[tmp[0][i]][j].slice(0, -1))) / 100,
                );
              else tmp[i + 2] += Number(EquipOpt[tmp[0][i]][j]);
            }
          }
        }
        ...
        if (Flag[7]) {
          if (Status["Job"][2] == 3 || Status["Job"][2] == 6)
            tmp[5] +=
              50 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 50 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 10)
            tmp[5] +=
              30 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 30 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 27)
            tmp[5] +=
              40 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 40 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else
            tmp[5] +=
              10 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 10 + tmp[5] + Status["Skill"][24][0] + Status["Skill"][24][1];
        } else tmp[5] = 100;
      ...
      case "Dodge": // [ 65] 回避
        ...
        tmp[1] =
          ...
          Status["AGI"][1] +
          Status["AGI"][2];
        if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 140)
          tmp[1] += 98;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 130)
          tmp[1] += 85;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 120)
          tmp[1] += 72;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 110)
          tmp[1] += 61;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 100)
          tmp[1] += 50;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 90)
          tmp[1] += 41;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 80)
          tmp[1] += 32;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 70)
          tmp[1] += 25;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 60)
          tmp[1] += 18;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 50)
          tmp[1] += 13;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 40)
          tmp[1] += 8;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 30)
          tmp[1] += 5;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 20)
          tmp[1] += 2;
        else if (Status["AGI"][0] + Status["AGI"][1] + Status["AGI"][2] >= 10)
          tmp[1] += 1;
        else tmp[1] += 0;
        tmp[2] +=
          Status["Job"][2] >= 11 && Status["Job"][2] <= 13
            ? 10 +
              Math.floor(Status["Skill"][6][0] + Status["Skill"][6][1] / 10)
            : 0; // クイックステップ
        ...
        if (Flag[7]) {
          if (Status["Job"][2] == 3 || Status["Job"][2] == 6)
            tmp[3] +=
              50 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 50 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 10)
            tmp[3] +=
              30 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 30 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 27)
            tmp[3] +=
              40 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 40 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else
            tmp[3] +=
              10 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 10 + tmp[3] + Status["Skill"][24][0] + Status["Skill"][24][1];
        } else tmp[3] = 100;
      ...
      case "CriRES": // [ 70] 被クリティカル発生率
        ...
        if (EquipOpt[tmp[0]])
          for (i = 0; i < EquipOpt[tmp[0]].length; i++)
            tmp[2] += Number(EquipOpt[tmp[0]][i]);
        $("Status_" + tmp[0]).innerHTML = Flag["5_5"]
          ? "-255"
          : tmp[1] + tmp[2];
        break;
      ...
      case "ATKSPD": // [ 73] 攻撃速度
        ...
        tmp[3] = Flag["14_3"]
          ? 5 + Math.floor((Number($("InBuff_0").value) / (100 / 6)) * 1.9)
          : 0; // グラーティア
        ...
        if (Flag[7]) {
          if (Status["Job"][2] == 3 || Status["Job"][2] == 6)
            tmp[4] +=
              50 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 50 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 10)
            tmp[4] +=
              30 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 30 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else if (Status["Job"][2] == 27)
            tmp[4] +=
              40 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 40 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1];
          else
            tmp[4] +=
              10 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1] >=
              100
                ? 100
                : 10 + tmp[4] + Status["Skill"][24][0] + Status["Skill"][24][1];
        } else tmp[4] = 100;
      ...
      case "ReduceMP": // [ 76] 消費MP
        ...
        tmp[1] =
          ...
          Math.floor(
            (Math.pow(
              Status["INT"][0] + Status["INT"][1] + Status["INT"][2],
              2,
            ) *
              2) /
              2000,
      ...
      case "Cooldown": // [ 79] 再使用時間
        ...
        tmp[1] =
          Math.floor(
            (Status["INT"][0] + Status["INT"][1] + Status["INT"][2]) / 10,
          ) * -2;
        if (EquipOpt[tmp[0]])
          for (i = 0; i < EquipOpt[tmp[0]].length; i++)
            tmp[2] += Number(EquipOpt[tmp[0]][i]);
      ...
      case "RESMagic": // [145] 魔術耐性
        tmp[0] = 145;
        tmp[1] +=
          Status["Job"][2] == 6 && Status["Equip"][1][0] % 10000 >= 1
            ? Math.floor((Status["Skill"][5][0] + Status["Skill"][5][1]) / 2)
            : 0; // パラディン：フィラクテリー
      ...
      case "RESPDebuff": // [146] 身体的異常耐性
        ...
        tmp[1] =
          Math.floor(
            (Status["STA"][0] + Status["STA"][1] + Status["STA"][2]) / 28,
          ) * 7;
      ...
      case "RESMDebuff": // [147] 精神的異常耐性
        ...
        tmp[1] =
          Math.floor(
            (Status["SPR"][0] + Status["SPR"][1] + Status["SPR"][2]) / 28,
          ) * 7;
    ...
    switch (a) {
      ...
      case "RESPDebuff": // [146] 身体的異常耐性
      case "RESMDebuff": // [147] 精神的異常耐性
        if (a == "RESFire" || a == "RESIce" || a == "RESLightning")
          if (Status["Job"][2] >= 25 && Status["Job"][2] <= 27)
            // ダークバインド
            for (i = 0; i <= 6; i++)
              tmp[1] +=
                Status["Equip"][i][0] % 10000 != 0 &&
                Name["Gem"][1][Status["Equip"][i][2]][0] == "闇" &&
                Status["Equip"][i][3] >= 1
                  ? 2 +
                    Math.floor(
                      (Status["Skill"][17][0] + Status["Skill"][17][1]) / 50,
                    )
                  : 0;
        if (a == "RESPoison" || a == "RESCharm" || a == "RESMagic")
          if (Status["Job"][2] >= 18 && Status["Job"][2] <= 20)
            // ホーリーディバイン
            for (i = 0; i <= 6; i++)
              tmp[1] +=
                Status["Equip"][i][0] % 10000 != 0 &&
                Name["Gem"][1][Status["Equip"][i][2]][0] == "光" &&
                Status["Equip"][i][3] >= 1
                  ? 2 +
                    Math.floor(
                      (Status["Skill"][12][0] + Status["Skill"][12][1]) / 50,
                    )
                  : 0;
        if (EquipOpt[tmp[0]])
          for (i = 0; i < EquipOpt[tmp[0]].length; i++)
      ...
      case "DAMDark": // [ 39] 闇オーラダメージ
        ...
        if (EquipOpt[tmp[0][0]]) {
          for (i = 0; i < EquipOpt[tmp[0][0]].length; i++) {
            ...
            else tmp[1][0] += Number(EquipOpt[tmp[0][0]][i]);
          }
        }
        ...
        if (Flag["8_1"]) {
          // フォックスグローブ
          tmp[4] = Math.floor(
            (Status["Skill"][8][0] + Status["Skill"][8][1]) / 20,
          );
          tmp[1][4] =
            1 +
            Math.ceil(Status["Lev"][0] * (100 / 300)) +
            Math.ceil(tmp[4] * (100 / 30));
          //            tmp[1][4] += // ビルトキシン未対応
  ...
  // 防御系 開かれている時だけ計算する
    ...
    switch (a) {
      case "LPRecSPD": // [  9] LP自然回復速度
        tmp[0] = 9;
        tmp[1] = Math.floor(
          (Status["STA"][0] + Status["STA"][1] + Status["STA"][2]) / 10,
        ); // 体力でのボーナス
        if ($("SelBuffClan_3").selectedIndex == 1)
          tmp[2] += 5; // フィジカルリカバリー
        else if ($("SelBuffClan_3").selectedIndex == 2) tmp[2] += 8;
        if ($("SelBuffClan_8").selectedIndex == 1)
          tmp[2] += 10; // ヒーリング
      ...
      case "MPRecSPD": // [ 10] MP自然回復速度
        tmp[0] = 10;
        tmp[1] = Math.floor(
          (Status["SPR"][0] + Status["SPR"][1] + Status["SPR"][2]) / 10,
        ); // 霊感でのボーナス
        tmp[2] += Flag["14_0"] ? 0 : 0; // ギブンリポーズ 未実装
        if ($("SelBuffClan_4").selectedIndex == 1)
          tmp[2] += 5; // フィジカルスピリット
      ...
      case "LPRec": // [ 12] LP自然回復量                   [ 14] 座っている時のLP回復量
        ...
        tmp[1] =
          Math.floor(Status["LP"] / 35) +
          Math.floor(
            (Status["STA"][0] + Status["STA"][1] + Status["STA"][2]) / 3,
          );
        for (i = 0; i < tmp[0].length; i++) {
          if (EquipOpt[tmp[0][i]])
            for (j = 0; j < EquipOpt[tmp[0][i]].length; j++)
      ...
      case "MPRec": // [ 13] MP自然回復量                   [ 15] 座っている時のMP回復量
        ...
        tmp[1] =
          Math.floor(Status["MP"] / 25) +
          Math.floor(
            (Status["SPR"][0] + Status["SPR"][1] + Status["SPR"][2]) / 3,
          );
        for (i = 0; i < tmp[0].length; i++) {
          if (EquipOpt[tmp[0][i]])
            for (j = 0; j < EquipOpt[tmp[0][i]].length; j++)
      ...
      case "LPsec": // [ 16] 2秒毎のLP回復量
        ...
        tmp[1] += Flag["13_7"]
          ? 15 +
            Math.floor(Number($("InBuff_0").value) / 10) +
            Math.floor((Status["Skill"][13][0] + Status["Skill"][13][1]) / 10)
          : 0; // リクライム
        tmp[1] += Flag["13_9"]
          ? 7 +
            Math.floor(Number($("InBuff_0").value) / 5) +
            Math.floor((Status["Skill"][13][0] + Status["Skill"][13][1]) / 10)
          : 0; // リストレーション
        if (EquipOpt[tmp[0]])
          for (i = 0; i < EquipOpt[tmp[0]].length; i++)
      ...
      case "RESStun": // [149] 気絶耐性                       [164] 気絶の耐性段階
        ...
        tmp[1] +=
          Status["Skill"][11][0] + Status["Skill"][11][0] >= 26 ? 15 : 0; // ラムセンス
        if (EquipOpt[tmp[0][0]])
          for (i = 0; i < EquipOpt[tmp[0][0]].length; i++)
            tmp[1] += Number(EquipOpt[tmp[0][0]][i]);
        $("Status_" + tmp[0][0]).innerHTML = tmp[1];
  ...
  if (Flag[5]) {
    ...
    switch (a) {
      case "FirstAid": // ファーストエイド
        tmp =
          Status["Job"][2] == 16 && Status["Lev"][0] >= 50
      ...
      case "Restoration": // リストレーション
        $("ViewHeal_4_1").innerHTML =
          7 + Math.floor(Ban / 10) + Math.floor(SPR / 5);
        break;
      ...
      case "Velox": // グラーティア
        ...
        document.getElementById("ViewBuff_0_1").innerHTML =
          "+" + (5 + Math.floor(tmp[0] * 1.9));
      ...
      case "LapisMedio": // ラピスメディオ
        ...
        document.getElementById("ViewBuff_1_1").innerHTML =
          "+" + (5 + tmp[0] + tmp[1]);
        document.getElementById("ViewBuff_1_3").innerHTML = tmp[2];
        break;
      case "GroupBarrier": // ウィンクルム
        document.getElementById("ViewBuff_2_1").innerHTML =
          "-" + (10 + Math.floor(Ble / 5));
        break;
      case "BlessWeapon": // ホーリーウェポン
        tmp[0] = Math.floor(SPR / (100 / 6));
        tmp[1] = Math.ceil((tmp[0] + 1) * (100 / 6));
        document.getElementById("ViewBuff_3_1").innerHTML =
          tmp[0] + Name["Text"]["Skill"]["Text"][2][Flag[0]];
      ...
      case "Gloom": // グルーミーエフェクト
        ...
        document.getElementById("ViewBuff_4_1").innerHTML =
          tmp[0] + Name["Text"]["Skill"]["Text"][2][Flag[0]];
        document.getElementById("ViewBuff_4_3").innerHTML = tmp[1];
        break;
      case "Enchantment": // エンハンスエンチャント
        document.getElementById("ViewBuff_5_1").innerHTML =
          10 + Math.floor(Ble / (100 / 20));
        tmp[0] = SPR >= 101 ? 4 : Math.floor(SPR / (100 / 4));
        tmp[1] = Math.floor(SPR / (100 / 5));
        tmp[2] = Math.ceil((tmp[0] + 1) * (100 / 4));
        tmp[3] = Math.ceil((tmp[1] + 1) * (100 / 5));
        document.getElementById("ViewBuff_6_1").innerHTML =
          tmp[0] + tmp[1] + Name["Text"]["Skill"]["Text"][2][Flag[0]];
        document.getElementById("ViewBuff_6_3").innerHTML =
          tmp[2] < tmp[3] ? tmp[2] : tmp[3];
        break;
      case "MinstrelSong": // 詩人の歌
        tmp[0] = Math.floor(SPR / (100 / 13));
        tmp[1] = Math.floor(Hym / (100 / 13));
        tmp[2] = Math.ceil((tmp[0] + 1) * (100 / 13));
        document.getElementById("ViewBuff_7_1").innerHTML =
          "+" + (5 + tmp[0] + tmp[1]);
      ...
      case "SageSong": // 魔力の歌
        ...
        document.getElementById("ViewBuff_9_1").innerHTML =
          tmp[0] + tmp[1] + Name["Text"]["Skill"]["Text"][2][Flag[0]];
      ...
      case "Toxify": // フォックスグローブ
        document.getElementById("ViewOther_0_1").innerHTML =
          Math.floor(Mix / (100 / 5)) +
          Name["Text"]["Skill"]["Text"][2][Flag[0]];
        break;
      case "ToxicAura": // ビルトキシン
        document.getElementById("ViewOther_1_1").innerHTML =
          Math.floor(Mix / (100 / 15)) +
          Name["Text"]["Skill"]["Text"][2][Flag[0]];
        break;
    }
  }
  ...
  if (Flag[6]) {
    switch (a) {
      case "POT_A":
        document.getElementById("ViewPOT_0_1").innerHTML = Math.floor(
          (20 * Status["POT"]) / 100,
        ); // 肉塊
        document.getElementById("ViewPOT_1_1").innerHTML = Math.floor(
          (60 * Status["POT"]) / 100,
        ); // 痛み止め
        document.getElementById("ViewPOT_2_1").innerHTML = Math.floor(
          (120 * Status["POT"]) / 100,
        ); // 傷薬
        document.getElementById("ViewPOT_3_1").innerHTML = Math.floor(
          (180 * Status["POT"]) / 100,
        ); // 治療薬
        document.getElementById("ViewPOT_4_1").innerHTML = Math.floor(
          (450 * Status["POT"]) / 100,
        ); // 特治療薬
        document.getElementById("ViewPOT_5_1").innerHTML = Math.floor(
          (145 * Status["POT"]) / 100,
        ); // 神秘のポーション
        document.getElementById("ViewPOT_6_1").innerHTML = Math.floor(
          (10 * Status["POT"]) / 100,
        ); // 雪割草
        document.getElementById("ViewPOT_7_1").innerHTML = Math.floor(
          (25 * Status["POT"]) / 100,
        ); // 蓬莱草
        document.getElementById("ViewPOT_8_1").innerHTML = Math.floor(
          (200 * Status["POT"]) / 100,
        ); // カボチャクッキー
        document.getElementById("ViewPOT_9_1").innerHTML = Math.floor(
          (150 * Status["POT"]) / 100,
        ); // アスティアンキャンディ
        break;
      case "POT_B":
        document.getElementById("ViewPOT_10_1").innerHTML = Math.floor(
          (Status["LP"] * Math.ceil(5 * Status["POT"])) / 10000,
        ); // リボン付きシャンパン
        document.getElementById("ViewPOT_11_1").innerHTML = Math.floor(
          (Status["LP"] * Math.ceil(15 * Status["POT"])) / 10000,
        ); // 快復のポーション
        document.getElementById("ViewPOT_12_1").innerHTML = Math.floor(
          (Status["LP"] * Math.ceil(30 * Status["POT"])) / 10000,
        ); // 生命のポーション
        document.getElementById("ViewPOT_13_1").innerHTML = Math.floor(
          (Status["LP"] * Math.ceil(18 * Status["POT"])) / 10000,
        ); // テイワズ
        break;
    }
  }
